...
static int Write_To_PVS(unsigned int var, unsigned int s_id, unsigned int add, int v) {
  ...
  for(i = 1; i <= NUM_OF_PVS; i++) {
    ret = TCP_Write(sock[i], mess, nBytes);
    if(ret <= 0) {
...
static void *socketReadThread(void *arg) {
  ...
  int i, ret, val, slave, function, adr, buflen, doit, remaining_bytes;
    ...
    if(select(FD_SETSIZE, &active_fd_set, NULL, NULL, NULL) > 0) {
      for(i = 1; i <= NUM_OF_PVS; i++) {
        if(FD_ISSET(sock[i], &active_fd_set)) {
          ret = TCP_Read(sock[i], temp, sizeof(scada_message));
          if(ret < 0) {
          ...
          ret = TCP_Read(sock[i], &temp[sizeof(scada_message)], remaining_bytes);
          if(ret < 0) {
            perror("Reading error");
            close(sock[i]);
            break;
...
int main(int argc,char *argv[])
{
  ...
  // connect to PVSs
  struct sockaddr_in pvs[NUM_OF_PVS + 1];
  pthread_t tid[2];
  ...
  for(i = 1; i <= NUM_OF_PVS; i++) {
    ...
    bzero(&pvs[i], sizeof(pvs[i]));
    pvs[i].sin_family = AF_INET;
    pvs[i].sin_port = htons(PORT);
    pvs[i].sin_addr.s_addr = inet_addr(pvs_list[i]);

    if(connect(sock[i], (struct sockaddr *)&pvs[i], sizeof(pvs[i])) < 0) {
