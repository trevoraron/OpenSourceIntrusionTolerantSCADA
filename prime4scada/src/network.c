...
void Initialize_Listening_Socket()
{
  ...
  /* Includes a blocking call in order to start server's 
   * operation just after the connection of a client.  */
  len = sizeof(client_addr);
  if((connfd = accept(NET.listen_sd, (struct sockaddr *)&client_addr, &len)) < 0) {
    perror("accept");
    exit(0);
  }
...
//  The next two lines are commented out for Prime demo
...
void Net_Srv_Recv(channel sk, int source, void *dummy_p) 
{
  ...
  /* Initialize a connection to the client (for Prime demo) */
  if(mess->type == INIT) {
