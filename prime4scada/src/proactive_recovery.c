...
void RECOVERY_Deliver_Ckpt_Digest_Reply(signed_message *mess) {
  ...
  if(OPENSSL_RSA_Digests_Equal(DATA.STATE.received_digest, DATA.STATE.digest) /*&& VAR.My_Server_ID != 4*/) {
...
void RECOVERY_Initialize_Catch_Up_Struct() {
  ...
  for(i = 1; i <= NUM_SERVERS; i++) {
    DATA.CAT.seq_num[i] = 0;
    DATA.CAT.view[i]    = 0;
    DATA.CAT.aru[i]     = 0;
  }
  for(i = 1; i <= NUM_SERVERS; i++)
    for(j = 1; j <= NUM_SERVERS; j++)
      DATA.CAT.temp_aru[i][j] = 0;
}
...
void RECOVERY_Retrieve_ORD_Cert(int32u pc_seq_num) {
  ...
  cert->val_attempt       = 0;
  for(i = 1; i <= NUM_SERVERS; i++)
    UTIL_Bitmap_Clear(&cert->bitmask, i);
}

void RECOVERY_Send_ORD_Cert_Request() {
  retrieved_cert *cert;
  signed_message *ord_cert;
  int32u dest_bits, i, count = 0;
...
void RECOVERY_Deliver_ORD_Cert_Request(signed_message *mess) {
  ...
  int32u i, dest_bits, count = 0, offset = 0;
