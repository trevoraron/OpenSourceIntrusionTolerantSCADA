...
void UTIL_Respond_To_Client(int32u machine_id, int32u time_stamp, int32u message_type,
                            int32u client_seq_num, byte content[UPDATE_SIZE])
{
  signed_message *mess;
  
  mess = ORDER_Construct_Client_Response(machine_id, time_stamp, message_type, 
                                         client_seq_num, content);
...
void UTIL_Write_Client_Response(signed_message *mess)
{
  ...
  // The following code has been commented out for the Prime demo.
...
*/
  ...
  ret = TCP_Write(NET.client_sd[VAR.My_Server_ID], mess,
                  UTIL_Message_Size(mess));
