////////////////////////////////////////////////////////////////////////////
//
// show_mask1 for ProcessViewServer created: Mon Apr 6 14:32:23 2015
//
////////////////////////////////////////////////////////////////////////////
#include "pvapp.h"
...
static int generated_defineMask(PARAM *p, DATA *d)
{
  int w,h,depth;

  if(p == NULL) return 1;
  if(d == NULL) return 1;
  if(d->mask == NULL) return 1;
  w = h = depth = strcmp(toolTip[0],whatsThis[0]);
  if(w==h) depth=0; // fool the compiler
  //pvStartDefinition(p,ID_END_OF_WIDGETS);
  //pvEndDefinition(p);
  //NEW STUFF TO SEND MASK
  char buf[80];
  int i = 0;
  int i_buf = 0;
  char i_c = d->mask[i];
  //loop thru and send each line in the mask 1 at a time
  while(i_c != '\0') {
    buf[i_buf++] = i_c;
    if(i_c == '\n') {
        i_buf = 0;
...
static int defineMask(PARAM *p, DATA *d)
{
  if(p == NULL) return 1;
  if(d == NULL) return 1;
  generated_defineMask(p,d);
